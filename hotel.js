class Hotel {
  ...
  listGuestInRoom(roomNo) {
    const floor = +roomNo.toString().slice(0, 1)
    const roomNumber = roomNo.toString().slice(1)
    console.log(this.rooms[floor - 1][roomNumber]["user"]["name"])
  }
  ...
  book(room, name, age) {
    ...
    if (this.rooms[floor - 1][roomNumber].status !== "checkin") {
      ...
      const foundFirstAvailableKeyCard = this.keycards.findIndex(keyCard => !keyCard.room)
      this.keycards[foundFirstAvailableKeyCard].room = room
      console.log(`Room ${room} is booked by ${name} with keycard number 1.`)
    } else {
      console.log(
        `Cannot book room ${floor + roomNumber} for ${name}, The room is currently booked by ${
          this.rooms[floor - 1][roomNumber].user.name
        }.`
      )
    }
  }
